...
float SynDic::calcConceptSim(const std::string &w1 ,const std:: string &w2){
    ...
    if (!ce1||!ce2)return 0.32;
        ...
        float result1=log(DP+conceptdic_.firstTypeDic[ce1->firstType]->secondTypeDic[ce1->secondType]->thirdTypeDic[ce1->thirdType]->wordGroupDic[ce1->wordGroup]->atomWordGroupDic[ce1->atomWordGroup]->node_num);
        float result2=log(DP+conceptdic_.firstTypeDic[ce2->firstType]->secondTypeDic[ce2->secondType]->thirdTypeDic[ce2->thirdType]->wordGroupDic[ce2->wordGroup]->atomWordGroupDic[ce2->atomWordGroup]->node_num);
        ...
        if (!(ce1->firstType == ce2->firstType))
            ...
            if (!(ce1->secondType == ce2->secondType))all=log(DP+conceptdic_.firstTypeDic[ce2->firstType]->node_num);
            else {
                if (!(ce1->thirdType == ce2->thirdType))all=log(DP+conceptdic_.firstTypeDic[ce2->firstType]->secondTypeDic[ce2->secondType]->node_num);
                else {
                    if (!(ce1->wordGroup == ce2->wordGroup))all=log(DP+conceptdic_.firstTypeDic[ce2->firstType]->secondTypeDic[ce2->secondType]->thirdTypeDic[ce2->thirdType]->node_num);
                    else{
                        if (!(ce2->atomWordGroup==ce1->atomWordGroup))all= log(DP+conceptdic_.firstTypeDic[ce2->firstType]->secondTypeDic[ce2->secondType]->thirdTypeDic[ce2->thirdType]->wordGroupDic[ce2->wordGroup]->node_num);
                        else
                            return 1.0;
